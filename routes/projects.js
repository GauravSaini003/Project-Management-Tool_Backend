...
// This is for the logging (activity log) functionality
...
router.post(
  ...
  async (req, res) => {
    try {
      const { project_name, deadline, status, assigned_members, company_name } =
        req.body;
      const created_by = req.user.id;
      ...
      if (existingUsers.length !== assigned_members.length) {
        return res
          .status(400)
          .json({ message: "One or more assigned members do not exist" });
      }
      ...
      await ActivityLog.create({
        user_id: req.user.id, // Creator
        board_id: newBoard.id, // New board ID
        task_id: null, // No task for this action
        action_type: "create", // Action type for creating a project
        description: `Created project "${project_name}"`,
      });
  ...
  async (req, res) => {
    try {
      ...
      if (!Array.isArray(members) || members.length === 0) {
        return res
          .status(400)
          .json({ message: "Provide at least one member to add." });
      }
      ...
      await project.addAssignedUsers(members); // Many-to-Many relation
       ...
       // Log activity after successfully adding members
       await ActivityLog.create({
        user_id: req.user.id, // Admin or Manager who added members
        board_id: project.id, // The project being updated
        task_id: null, // No task for this action
        action_type: "assign", // Action type for assigning members
        description: `Added ${members.length} members to project "${project.project_name}"`,
      });
...
// ✅ Delete a Project this is the permanently delete a project but we are now using soft delete (Only ADMIN)
// router.delete("/:id", authMiddleware, roleMiddleware(["ADMIN"]), async (req, res) => {
//     try {
//         const BoardId = req.params.id;
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      await ProjectAssignedUser.update(
        { deleted_at: new Date() },
      ...
      await board.destroy();
        ...
        // Log activity for soft deleting the project
      await ActivityLog.create({
        user_id: req.user.id, // Admin who deleted the project
        board_id: board.id, // The project being deleted
        task_id: null, // No task for this action
        action_type: "delete", // Action type for deleting
        description: `Moved project "${board.project_name}" to trash`,
      });
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      await Task.update(
        { deleted_at: null },
        { where: { boardId: boardId }, paranoid: false }
      );

      // ✅ Restore project assignments
      await ProjectAssignedUser.update(
        { deleted_at: null },
        { where: { project_id: boardId }, paranoid: false }
      );

     // Log activity after restoring project
     await ActivityLog.create({
      user_id: req.user.id, // Admin restoring the project
      board_id: board.id, // The project being restored
      task_id: null, // No task for this action
      action_type: "restore", // Action type for restoring
      description: `Project "${board.project_name}" was restored`,
    });
...
//       await ProjectAssignedUser.destroy({
//         where: { project_id: boardId },
//         force: true,
//       });

//  // Manually delete related activity logs
//  await ActivityLog.destroy({ where: { board_id: boardId } });

//       // Permanently delete the project
//       await board.destroy({ force: true });

      
//     // ✅ Log activity after permanent delete
//     await ActivityLog(req.user.id, `Permanently deleted project "${board.project_name}"`, {
//       board_id: board.id,
//       action_type: 'delete',
//       description: `Permanently deleted project "${board.project_name}"`,
//     });
...
// ✅ Permanently delete project (including activity logs, tasks, and assignments)
router.delete(
  "/trash/:id",
  authMiddleware,
  roleMiddleware(["admin"]),
  async (req, res) => {
    try {
      ...
      // Find the board (project)
      const board = await Board.findOne({
        where: { id: boardId },
        paranoid: false, // Important: Allow soft-deleted projects to be found
      });
      ...
      // 1. Delete related activity logs
      ...
      // 3. Delete project assignments (users assigned to the project)
      await ProjectAssignedUser.destroy({
        where: { project_id: boardId },
        force: true,
      });

      // 4. Permanently delete the project itself
      await board.destroy({ force: true });
  ...
  async (req, res) => {
    try {
      ...
      if (!Array.isArray(members) || members.length === 0) {
        return res
          .status(400)
          .json({ message: "Provide at least one member to remove." });
      }
      ...
      res
        .status(500)
        .json({ message: "Error removing members", error: error.message });
    }
  }
);
